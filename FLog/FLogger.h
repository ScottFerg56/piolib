...
#define flogd(format, ...) FLogger::flog.flog_printf(FLOG_FORMAT(D, format), ##__VA_ARGS__)
...
public:
    static FLogger flog;
    static int flog_printfv(const char *format, va_list);
    static int flog_printf(const char *format, ...);
